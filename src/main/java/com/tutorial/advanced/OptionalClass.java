...
public class OptionalClass {
    ...
    private static void demonstrateOptionalCreation() {
        ...
        try {
            Optional<String> nonNullOptional = Optional.of("Non-null value");
        ...
        Optional<String> nonNullOptional2 = Optional.ofNullable(nonNullValue);
        ...
        System.out.println("\nCreating Optional from method return:");
        Optional<String> methodOptional = findUserById(1);
        System.out.println("  Method return: " + methodOptional);
        
        Optional<String> methodOptional2 = findUserById(999);
    ...
    private static void demonstrateValueTransformation() {
        ...
        Optional<String> mapped = presentOptional.map(String::toUpperCase);
        System.out.println("  Original: " + presentOptional);
        System.out.println("  Mapped: " + mapped);
        
        Optional<String> mappedEmpty = emptyOptional.map(String::toUpperCase);
        System.out.println("  Empty mapped: " + mappedEmpty);
        
        // map() with multiple transformations
        Optional<String> multipleMapped = presentOptional
            .map(String::trim)
        ...
        System.out.println("\nUsing flatMap():");
        Optional<String> flatMapped = presentOptional
            .flatMap(s -> Optional.of(s.trim().toUpperCase()));
        System.out.println("  Flat mapped: " + flatMapped);
        
        // filter() - filters values based on condition
        System.out.println("\nUsing filter():");
        Optional<String> filtered = presentOptional
            .filter(s -> s.length() > 5);
        System.out.println("  Filtered (length > 5): " + filtered);
        
        Optional<String> filteredEmpty = presentOptional
            .filter(s -> s.length() > 100);
        System.out.println("  Filtered (length > 100): " + filteredEmpty);
        
        // Complex transformation chain
        System.out.println("\nComplex transformation chain:");
        Optional<String> complex = presentOptional
            .map(String::trim)
            .filter(s -> !s.isEmpty())
            .map(String::toUpperCase)
            .map(s -> s.replace(" ", "_"))
    ...
    private static void demonstrateOptionalChaining() {
        ...
        System.out.println("\nChaining with flatMap:");
        Optional<String> flatMapResult = Optional.of("test")
            .flatMap(s -> Optional.of(s.toUpperCase()))
            .flatMap(s -> Optional.of(s + "_SUFFIX"));
    ...
    private static void demonstrateRealWorldExamples() {
        ...
        Optional<User> userNotFound = userService.findUserById(999);
        ...
        Optional<String> content = processor.readFile("test.txt");
        content.ifPresent(c -> {
        ...
        Optional<ApiResponse> response = apiClient.getUserData(1);
    ...
    static class Configuration {
        ...
        public Optional<String> getDatabaseHost() {
            return Optional.ofNullable(config.get("db.host"));
        }
    ...
    static class FileProcessor {
        public Optional<String> readFile(String filename) {
            ...
            if ("test.txt".equals(filename)) {
                return Optional.of("This is test file content");
            }
            return Optional.empty();
        }
    }
    ...
    static class ApiClient {
        public Optional<ApiResponse> getUserData(int userId) {
            // Simulate API call
            if (userId == 1) {
                return Optional.of(new ApiResponse("User data", 200));
            }
            return Optional.empty();
        }
    }
